<div align="center">
  ...
  [![Zed v1.12.1](https://img.shields.io/badge/Zed-v1.12.1-blue?logo=zedindustries&logoColor=white)](https://github.com/zed-industries/zed/releases/tag/v1.12.1)
...
本仓库是 [LI-NA/zed-i18n](https://github.com/LI-NA/zed-i18n) 的 fork，而不是向上游提交补丁，因为这里的目标是个人化的默认配置，并不适合通用的上游项目：

- **只编译中文、只编译 Linux。** 上游会为 macOS、Windows、Linux 三平台构建全部 13 种语言；本仓库只需要 Linux（x86_64 + aarch64）上的 `zh-CN`，能大幅缩短 CI 时间、降低成本。若直接改上游的默认值，会破坏其他用户的使用场景。
- **增加 `.deb` 打包。** 上游的 Linux 产物只有 `tar.gz`；本仓库额外生成 `.deb` 包，便于在 Debian/Ubuntu 系统上干净地安装和升级。
- **兼容更老的 glibc。** x86_64 使用 `ubuntu-22.04` runner 编译（而非更新的镜像），使二进制能在较老的发行版上运行；aarch64 保留 `ubuntu-24.04-arm`，因为 22.04 的 ARM runner 在链接阶段会资源不足。
- **全自动发布。** 推送 `v*-i18n.*` 标签即可端到端完成构建、打包并发布公开（非草稿）的 GitHub Release，无需任何手动发布步骤。
- **每日同步上游。** 定时工作流每天检查上游是否有新版本，自动合并（冲突时开 Issue 提醒）、运行测试并打标签触发新构建——中文版可以自动跟上上游更新，无需人工干预。

这些改动无法推送到原仓库（没有写权限），而且大多是带个人偏好的默认配置、并非上游需要的通用改进，因此单独维护一个仓库是最干净的方式。
